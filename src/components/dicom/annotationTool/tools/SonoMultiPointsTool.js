...
import drawBezier from '../util/pen/drawBezier'

//util
const triggerEvent = cornerstoneTools.import('util/triggerEvent')
// const moveHandleNearImagePoint = cornerstoneTools.import('util/moveHandleNearImagePoint')
...
import insertOrDelete from '../util/pen/insertOrDelete'
import freehandArea from '../util/freehand/freehandArea'
import calculateFreehandStatistics from '../util/freehand/calculateFreehandStatistics'
import freehandIntersect from '../util/freehand/freehandIntersect'
import FreehandHandleData from '../util/freehand/FreehandHandleData'

// const { hideToolCursor, setToolCursor } = cornerstoneTools.import('store/setToolCursor')
// const hideToolCursor = cornerstoneTools.import('store/hideToolCursor')
// const setToolCursor = cornerstoneTools.import('store/setToolCursor')
import { multiPointsCursor } from '../cursors/index.js';
import utils from '../util/pen/utils'

const logger = getLogger('tools:annotation:SonoMultiPointsTool');


/**
 * @public
 * @class SonoMultiPointsTool
 * @memberof Tools.Annotation
 * @classdesc 多点工具
 * @extends Tools.Base.BaseAnnotationTool
 */
export default class SonoMultiPoints extends BaseAnnotationTool {
  constructor(props = {}) {
    const defaultProps = {
      name: 'SonoMultiPoints',
      supportedInteractionTypes: ['Mouse'],
      configuration: defaultFreehandConfiguration(),
      svgCursor: multiPointsCursor,
    };
  ...
  _findLineNearPoint (element, data, coords) {
    if (!data) {
      return;
    }
    let allPoints = utils.getLinePoint(data);
    if (!allPoints.length) {
      return;
    }
    for (let item of allPoints) {
      ...
      if (external.cornerstoneMath.point.distance(handleCanvas, coords) < 12) {
        return item;
      }
    }
  }
  ...
  pointNearTool(element, data, coords) {
    // console.log(getToolState(element, this.name))
    // console.log(data)
    ...
    const isPointNearTool = this._findLineNearPoint(element, data, coords);
  ...
  renderToolData(evt) {
      ...
      draw(context, context => {
        ...
        if (data.handles.points.length) {
          for (let j = 0; j < data.handles.points.length; j++) {
            ...
            // 每一条线都需要重画
            // 传入当前鼠标位置，否则断线
            // drawBezier(context, element, data.handles.points, j, mousePoint, {
            //     color,
            // });
    ...
    /**
     * 标注时，在信息中添加入tag，加入textBox，显示在图像上
     * @param {*} data 当前标注后的数据（原始信息）
     */
  ...
  preMouseDownCallback(evt) {
    ...
    const nearby = this._pointNearHandleAllTools(element, coords);
   ...
   * @returns {Object}
   */
  _pointNearHandleAllTools(element, coords) {
    // const { currentPoints, element } = eventData;
    // const coords = currentPoints.canvas;
    const toolState = getToolState(element, this.name);
  ...
  _getMouseLocation(eventData) {
    ...
    config.mouseLocation.handles.start.x = Math.min(Math.max(0, config.mouseLocation.handles.start.x), image.width)
    config.mouseLocation.handles.start.y = Math.min(Math.max(0, config.mouseLocation.handles.start.y), image.height)
  }
...
function defaultFreehandConfiguration() {
  return {
    ...
    spacing: 1,
    activeHandleRadius: 1,
    completeHandleRadius: 0,
